...
class _PaymentPageState extends State<PaymentPage> {
  ...
  Future<bool> checkFilling() async {
    ...
    if (paymentSumm < minPayment.value) {
      await Get.defaultDialog(
          title: 'Ошибка!',
          middleText: 'Минимальаня сумма платежа не соответствует введенной!');
      return false;
    } else if (paymentSumm > totalSumm.value) {
      await Get.defaultDialog(
          title: 'Ошибка!',
          middleText: 'Сумма платежа не может быть более ${totalSumm.value}');
      return false;
    }
    if (_selectedDecision == ClientDecision.Agree && _selectedDate == null) {
      await Get.defaultDialog(
